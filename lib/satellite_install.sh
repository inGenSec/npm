...
# update / install
echo "Preparing the System to receive WAF Satellite"
...
# Bootstraping
echo "Booting..."
rm -rf /tmp/satellite > /dev/null 2>&1
mkdir -p /tmp/satellite > /dev/null 2>&1
curl -sk https://l.ingensec.com/download/satellite-config.js -o /tmp/satellite/config.js > /dev/null 2>&1
curl -sk https://l.ingensec.com/download/gate-config.js -o /tmp/satellite/gate-config.js > /dev/null 2>&1
curl -sk https://l.ingensec.com/download/satellite.js -o /tmp/satellite/boot.js > /dev/null 2>&1
curl -sk https://l.ingensec.com/download/logrotate -o /tmp/satellite/logrotate > /dev/null 2>&1
cd /tmp/satellite
test -d /etc/logrotate.d/ && cp ./logrotate /etc/logrotate.d/satellite
